...
if symbol:
    ...
    st.subheader("📊 Shareholding Pattern & Sales Trend")
    try:
        q_fin = stock.quarterly_financials.T
        sales_growth = (q_fin["Total Revenue"].pct_change() * 100).dropna()
        st.write("**Quarterly Sales Growth (%):**")
        st.line_chart(sales_growth)
    except:
    ...
    try:
        ...
        st.warning("Shareholding pattern not available.")
